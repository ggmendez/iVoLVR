...
THREE.glTFLoader.prototype.load = function( url, callback ) {
    ...
    IndicesDelegate.prototype.handleError = function(errorCode, info) {
        // FIXME: report error
        if (LOG) console.log("ERROR(IndicesDelegate):"+errorCode+":"+info);
    };
    ...
    VertexAttributeDelegate.prototype.handleError = function(errorCode, info) {
        // FIXME: report error
        if (LOG) console.log("ERROR(VertexAttributeDelegate):"+errorCode+":"+info);
    };
    ...
    AnimationParameterDelegate.prototype.handleError = function(errorCode, info) {
        // FIXME: report error
        if (LOG) console.log("ERROR(AnimationParameterDelegate):"+errorCode+":"+info);
    };
    ...
    InverseBindMatricesDelegate.prototype.handleError = function(errorCode, info) {
        // FIXME: report error
        if (LOG) console.log("ERROR(InverseBindMatricesDelegate):"+errorCode+":"+info);
    };
    ...
    ShaderDelegate.prototype.handleError = function(errorCode, info) {
        // FIXME: report error
        if (LOG) console.log("ERROR(ShaderDelegate):"+errorCode+":"+info);
    };
    ...
    var ThreeGLTFLoader = Object.create(glTFParser, {
        ...
        createShaderMaterial : {
        	value: function(material) {
        		...
        		if (!fragmentShader) {
                    if (LOG) console.log("ERROR: Missing fragment shader definition:", material.params.fragmentShader);
            		return new THREE.MeshPhongMaterial;
        		}
        		
        		var vertexShader = theLoader.shaders[material.params.vertexShader];
        		if (!fragmentShader) {
                    if (LOG) console.log("ERROR: Missing vertex shader definition:", material.params.vertexShader);
            		return new THREE.MeshPhongMaterial;
        		}
...
//               		texture.minFilter = THREE.LinearFilter;
        ...
        handleMesh: {
            value: function(entryID, description, userInfo) {
                ...
                if (!primitivesDescription) {
                    ...
                    if (LOG) console.log("MISSING_PRIMITIVES for mesh:"+ entryID);
                    return false;
                }
                                    ...
                                    if (threeMesh && dobones) {
        	                            ...
        	                            for (i = 0; i < len; i++) {
        	                                ...
        	                                if (joint) {
        	                                    ...
        	                                    if (LOG) console.log("WARNING: jointId:"+jointId+" cannot be found in skeleton:"+skeleton);
        	                                }
        	                            }
        ...
        handleScene: {
            value: function(entryID, description, userInfo) {
                ...
                if (!description.nodes) {
                    if (LOG) console.log("ERROR: invalid file required nodes property is missing from scene");
                    return false;
                }
        ...
        createAnimations : {
        	value : function() {
        		for (var name in this.nodeAnimationChannels) {
        			...
        			//if (LOG) console.log(" animation channels for node " + name);
        			//for (i = 0; i < len; i++) {
        			//	if (LOG) console.log(nodeAnimationChannels[i]);
        			//}
        ...
        handleAnimation: {
            value: function(entryID, description, userInfo) {
	            ...
	            if (!parameters) {
	                //FIXME: not implemented in delegate
	                if (LOG) console.log("MISSING_PARAMETERS for animation:"+ entryID);
	                return false;
	            }
