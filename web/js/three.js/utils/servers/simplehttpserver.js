...
function handleRequest(request, response) {
	...
	if (LOG) console.log('[' + (new Date()).toUTCString() + '] ' + '"' + request.method + ' ' + pathname + '"');
...
require('dns').lookup(require('os').hostname(), function (err, addr, fam) {
 	if (LOG) console.log('Running at http://' + addr  + ((port === 80) ? '' : ':') + port + '/');
})

if (LOG) console.log('Three.js server has started...');
if (LOG) console.log('Base directory at ' + currentDir);