var ShapeData = fabric.util.createClass(fabric.Path, {
   initialize: function (options) {
      ...
      this.set('value', createShapeValue(options.shape || CIRCULAR_MARK, null));
...
//                var shapeValue = theDataType.createValue(SVGPATHGROUP_MARK, svgPathGroupMark);
      ...
      outputShapeSelector.on('change', function (e) {
         ...
         if (newShapeType == SVGPATHGROUP_MARK) {
            ...
            inputField.change(function (evt) {
               ...
               if (LOG) console.log(inputField);

               var filename = $(this).val();
               selectedFileName = filename;

               selectedFile = this.files[0];
               if (LOG) console.log("selectedFile:");
               if (LOG) console.log(selectedFile);
            });
