...
//                var arrayNode = createXMLElement("array"); // TODO: It would be better if the created node is still a value one, but with type array
    ...
    this.setValue = function (value, renderCanvas, shouldAnimate) {
        ...
        if (parentObject && (parentObject.isMark || parentObject.isVixor || parentObject.isNumericCollectionGenerator)) {
            ...
            // TODO: At the moment, the setProperty methods of both Marks and Vixors is responsible to inform the outgoing connections about the new value
            ...
            // deal with the other elements of the interface. As in the DataType class, this should be done only if the setProperty vas successful (i.e. it returned true)
    ...
    this.createValue = function () {
        ...
        if (theVisualProperty.attribute === "fill") {
            ...
            if (theVisualProperty.attribute === "colorValues") {
                ...
                var samplerVixor = theVisualProperty.parentObject;
                samplerVixor.colorValues.forEach(function (color) {
                    if (LOG)
                        console.log("color:");
                    if (LOG)
                        console.log(color);
    ...
    this.createVisualDataType = function () {
        ...
        var dataType = new CreateDataType(options);
        return dataType;

    };

    this.applySelectedStyle = function (unSelectConnectors) {
        this.selected = true;
...
//            },
            ...
            'newInConnection': function (options) {
                ...
                if (LOG)
                    console.log("incommingDataType[theVisualProperty.dataTypeProposition]:");
                if (LOG)
...
//
//
//                /*theVisualProperty.parentObject.setProperty(targetAttribute, updatedValue, inConnection.source, shouldAnimate);
